...
TypeScript types for all request/response payloads live in **`api-types.ts`** at the project root.
...
Use **`api-types.ts`** for exact TypeScript types of all request bodies and response `data` shapes.
